...
(function(){ 'use strict';
  ...
  // Assuming you have a basic Snake game structure where the snake moves in an array
...
// Create a function to draw the snake with colors for head and body
function drawSnake() {
  const snakeContainer = document.getElementById('gameArea'); // Assuming you have a game container

  // Clear any previous snake segments
  snakeContainer.innerHTML = '';

  for (let i = 0; i < snake.length; i++) {
    const snakeSegment = document.createElement('div');
    snakeSegment.classList.add('snake'); // Apply base snake styles

    // Apply different styles for the head and tail
    if (i === 0) {
      snakeSegment.classList.add('snake-head'); // Style for snake head
    } else if (i === snake.length - 1) {
      snakeSegment.classList.add('snake-tail'); // Style for snake tail
    }

    // Position the segments based on their coordinates
    ...
    // Add the snake segment to the game area
    snakeContainer.appendChild(snakeSegment);
  }
}

// Call this function on each game loop to update the snake's position
